...
Sub wheel_create
	...
	Dim svsstep As Int 
	svsstep = 50dip
	Addres.Initialize2(svsstep,StrAddr,True,"addres")
...
Sub json_interpreter1 (jstr As String)
	...
	If map1.Get("pktype") = "reply" Then
		...
		End If 
	Else	
		Take_Address_Take_Pwm 'call suv take_address
	End If  	
	Catch
...
Sub FollowLuxOnOff
	...
	Else If Addres.ReadWheel = StrAddr(1) AND  Set.ReadWheel = "yes" AND FollowLuxOn_Off.ReadWheel = "FolOff" Then	
		string_invPing(7,StrAddr(1))
		'string_invPing(7,StrAddr(1))
	End If
	If Addres.ReadWheel = StrAddr(2) AND Set.ReadWheel = "yes" AND FollowLuxOn_Off.ReadWheel = "FolOn" Then
		string_invPing(6,StrAddr(2))
	Else If Addres.ReadWheel = StrAddr(2) AND  Set.ReadWheel = "yes" AND FollowLuxOn_Off.ReadWheel = "FolOff" Then	
		string_invPing(7,StrAddr(2))
		'string_invPing(7,StrAddr(2))
	End If
	If Addres.ReadWheel = StrAddr(3) AND Set.ReadWheel = "yes" AND FollowLuxOn_Off.ReadWheel = "FolOn" Then
		string_invPing(6,StrAddr(3))
	Else If Addres.ReadWheel = StrAddr(3) AND  Set.ReadWheel = "yes" AND FollowLuxOn_Off.ReadWheel = "FolOff" Then	
