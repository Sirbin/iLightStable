...
Sub back_Click
	...
	End If 
	Activity.Finish
	l.Clear ' cancel list with Mac Adress'
	StartActivity("main")
End Sub 
Sub tables
	'Creation Table 
...
Sub json_interpreter1 (jstr As String)
	...
	map1=json.NextObject
		...
		End If 
	Catch
		admin.Disable 'if a json mapping has a problem disconnect and call sub clock1'
...
Sub BT_StateChanged(NewState As Int,OldState As Int)
	'Controll if state of bluethoot device co
	If NewState = admin.STATE_ON Then
		Connect_3box ' call sub connect_3box 
		Log("BT Connect")
	Else 
		serial1.Disconnect 
		Log("BT Disconnect")
...
Sub Serial1_Connected (success As Boolean)
 Dim msg As String 
	If success = True Then
		ToastMessageShow("Bluetooth connecd with success" ,True)
...
Sub Connect_3box 
	...
	If PariredDevices.ContainsKey(MyDevice) Then 
		...
		Log(l)
	End If 
	serial1.Connect(l.Get(0)) ' connect with 3box address
End Sub	
